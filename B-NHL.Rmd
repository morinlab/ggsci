...
include_nhl=TRUE
  ...
  clin_df = data.frame(Clinical=factor(names(clin_cols),levels=rev(names(clin_cols))),hex=clin_cols) %>% filter(!Clinical %in% c("M","F"))
   clin = ggplot(clin_df,aes(x=Clinical,y=0,fill=hex,label=Clinical)) +
    geom_tile(width = 0.9, height = 1) +
    geom_text(color = "white",fontface="bold") +
    scale_fill_identity(guide = "none") +coord_flip() + theme_void() +
    labs(title="Clinical") + theme(plot.title = element_text(lineheight = 0.9,hjust=0.5,face="bold"))
  if(include_nhl){
    grid.arrange(nhl,coo,harvard,lymphgen,hmrn,clin,ncol = 6)
...
extra=c()
show_col(x="clinical",to_drop=c("M","F"),extra_cols = extra) #add colours that aren't in ggsci (yet)
```
